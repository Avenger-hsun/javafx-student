package org.kuro.student.utils;
...
public final class WindowUtils {
    ...
    /**
     * 为窗体primaryStage添加windows平台的样式风格，如聚焦时阴影效果加强，
     * 失去焦点时阴影效果减弱、放置居中和最大化时图标化恢复时窗口大小为可视化大小
     *
    ...
    public static void addWindowsStyle(Stage primaryStage) {
        ...
        primaryStage.focusedProperty().addListener((observable, oldValue, newValue) -> {
            if (!primaryStage.isMaximized()) { // 非最大化状态才执行
                // 取出shadowPane
                Pane shadowPane = (Pane) ((StackPane) primaryStage.getScene().getRoot()).getChildren().get(1);
                if (newValue) {
